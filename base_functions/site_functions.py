...
def process_photos(id: str, count_photos: str) -> list:
    ...
    try:
        ...
        pattern = r'{size}'
        list_photos = [
            data_photos[i]['baseUrl'] for i in range(0, int(count_photos))]
...
def get_distination(city: str) -> dict:
    ...
    try:
        logger.info(
            'Отправка запроса для получения отелей согласно расположению ')
        response = requests.request(
            "GET", url, headers=headers, params=querystring)
        result = json.loads(response.text)
        list_destinations = {}
...
def get_details(id, check_in, check_out, distance):
    ...
    querystring = {"id": id, "checkIn": check_in, "checkOut": check_out,
                   "adults1": "1", "currency": "USD",
                   "locale": "en_US"}
...
def post_to_tg(response, distance) -> str:
    ...
    suggestion['Общая стоимость'] = find_dict_key(
        response, 'fullyBundledPricePerStay')
    if suggestion['Общая стоимость'] is not None:
        suggestion['Общая стоимость'] = \
            suggestion['Общая стоимость'].split(' ')[1]
    else:
...
def list_hotels_by_destination(message):
    with bot.retrieve_data(message.from_user.id, message.chat.id) as data:
        ...
        querystring = {
            "destinationId": destination_id, "pageNumber": "1",
            "pageSize": "4", "checkIn": check_in, "checkOut": check_out,
            "adults1": "1", "sortOrder": sortorder,
